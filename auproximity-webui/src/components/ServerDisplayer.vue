<template>
	<v-card class="pa-5">
		...
		<div class="text-center">
			<h2>{{ title }}</h2>
			<h4 v-if="$store.state.joinedRoom">
				Current Map:
				{{
					["The Skeld", "Mira HQ", "Polus", "The Skeld", "Airship"][
						this.settings.map
					]
				}}
			</h4>
		...
		<div>
			<span v-for="(value, i) in remoteStreams" :key="i">
				<audio v-audio="value" class="d-none"></audio>
			</span>
		</div>
...
export default class ServerDisplayer extends Vue {
	...
	createPeer(uuid: string) {
				...
				if (typeof this.peer !== "undefined") {
					for (const c of this.$store.state.clients) {
						const call = this.peer.call(
							c.uuid,
							this.$store.state.mic.destStream.stream
						);
		...
		this.peer.on("call", async (call) => {
			if (
				this.$store.state.clients.find((c: ClientModel) => c.uuid === call.peer)
			) {
	...
	async setupMyStream() {
		if (!this.$store.state.mic.volumeNode) {
			...
			this.$store.state.globalGainNode.gain.value = this.$store.state.muted
				? 0
				: 1;
	...
	connectCall(call: Peer.MediaConnection) {
		return new Promise<void>((resolve) => {
			call.on("stream", (remoteStream) => {
				...
				const source = this.remotectx.createMediaStreamSource(
	...
	async onRemoveClient(payload: { uuid: string; ban: boolean }) {
		if (payload.uuid === this.$store.state.me.uuid) {
			this.showSnackbar = true;
			this.snackbarMessage = payload.ban
				? "You were banned from " + this.$store.state.backendModel.gameCode
				: "You were kicked from " + this.$store.state.backendModel.gameCode;
		...
		this.$store.state.clients = this.$store.state.clients.filter(
			(client: ClientModel) => {
	...
	recalcVolumeForRemoteStream(stream: {
		...
		const client: ClientModel = this.$store.state.clients.find(
			(c: ClientModel) => c.uuid === stream.uuid
		);
		if (!client) return;
	...
	get LERP_VALUE() {
		return this.$store.state.options.falloffVision
			? 2 + this.settings.crewmateVision
			: this.$store.state.options.falloff;
	}
}
</script>
